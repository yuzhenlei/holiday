...
class RegExp
{
    ...
    {
        return '/(\d{1,2}+)月(\d{1,2}+)日[至—](\d{1,2}+)月(\d{1,2}+)日[放假|调休]+/';
    }

    public static function holidayRangeT3()
    {
        return '/(\d{1,2}+)月(\d{1,2}+)日[至—](\d{1,2}+)日[放假|调休]+/';
    }

    public static function holidayRangeT4()
    {
        return '/(\d{1,2}+)月(\d{1,2}+)日[放假|调休]+/';
    }

    public static function holidayRangeT5()
    {
        return '/(\d{1,2}+)月(\d{1,2}+)日放假，(\d{1,2}+)月(\d{1,2}+)日.*?补休/';
    }
